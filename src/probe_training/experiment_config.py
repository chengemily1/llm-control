...
class BaseExperimentConfig:
    ...
    def get_processed_data_path(self, exp_dir: str) -> str:
        ...
        raise NotImplementedError(
            "Child classes must implement get_processed_data_path"
        )
...
class ElixExperimentConfig(BaseExperimentConfig):
    ...
    def validate_config(self):
        ...
        if self.user not in valid_users:
            raise ValueError(
                f"Invalid user type: {self.user}. Must be one of {valid_users}"
            )
...
class ReviewsExperimentConfig(BaseExperimentConfig):
    ...
    def validate_config(self):
        ...
        if self.reps_suffix not in valid_reps:
            raise ValueError(
                f"Invalid reps_suffix: {self.reps_suffix}. Must be one of {valid_reps}"
            )
    ...
    def get_processed_data_path(self, exp_dir: str) -> str:
        """Get the path to the processed data file."""
        return os.path.join(
            exp_dir, f"user_{self.user}", "data", f"{self.split}_processed.csv"
        )
...
@dataclass
class PersonaExperimentConfig(BaseExperimentConfig):
    """Configuration for Persona experiment."""

    # Data settings
    text_field: str = "instruction+data"  # Field containing the text
    label_field: str = (
        "score"  # Field containing the preference score. TODO JL 3/19/25 we construct this as cosine similarity after contrastive loss
    )

    # Dataset settings
    dataset_name: str = "SynthLabsAI/PERSONA"
    user: str = "temp"  # JL 3/19/25: temp user
    reps_suffix: str = "final"  # Which representation file to use (final or 5000)
    ...
    def get_processed_data_path(self, exp_dir: str) -> str:
        """Get the path to the processed data file."""
        return os.path.join(
            exp_dir, f"user_{self.user}", "data", "train_shuffled_balanced.csv"
        )
